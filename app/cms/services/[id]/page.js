    ...
    const fetchService = async () => {
      try {
        ...
        if (response.ok) {
          ...
          setFormData({
            pragma_link: service.pragma_link || "",
            bg_image: service.bg_image || "",
            title: service.title || "",
            description: service.description || "",
            keywords: service.keywords || [],
            explore_link: service.explore_link || "",
            card: {
              intro: service.card?.intro || "",
              icon: service.card?.icon || "",
              read_more: service.card?.read_more || "",
            },
            cta: {
              caption: service.cta?.caption || "",
              link: service.cta?.link || "",
            },
            process: {
              bg_image: service.process?.bg_image || "",
              title_first: service.process?.title_first || "",
              title_second: service.process?.title_second || "",
              desc: service.process?.desc || "",
              steps: service.process?.steps || [],
            },
            why_choose_us: {
              title: service.why_choose_us?.title || "",
              items: service.why_choose_us?.items || [],
            },
            parent_service: service.parent_service || "",
            bread_crumbs: service.bread_crumbs || [],
            active: service.active !== undefined ? service.active : true,
          });
        } else {
          setErrors({ fetch: "Failed to load service data" });
        }
      } catch (error) {
        setErrors({ fetch: "Network error occurred" });
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setErrors({ submit: errorData.error || "Failed to update service" });
      }
    } catch (error) {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const errorData = await response.json();
        setErrors({ submit: errorData.error || "Failed to delete service" });
      }
    } catch (error) {
      setErrors({ submit: "Network error occurred" });
    }
  ...
  return (
    <div>
      <div className="d-flex justify-content-between align-items-center mb-4">
        <h1 className="h3 mb-0">Edit Service</h1>
